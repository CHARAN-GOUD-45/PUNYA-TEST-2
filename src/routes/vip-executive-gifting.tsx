...
export const Route = createFileRoute("/vip-executive-gifting")({
  head: () => ({
    meta: [
      { title: "VIP & Executive Gifting — Punya Ventures" },
      { name: "description", content: "Premium VIP and executive gifts for distinguished recipients." },
    ],
  }),
  component: VIPExecutiveGiftingPage,
});

function VIPExecutiveGiftingPage() {
  const [sort, setSort] = useState("featured");

  const filtered = useMemo(() => {
    let r = products.filter((p) => p.category === "Executive Gifting" || p.recipient.includes("Executives"));
    ...
    <div>
      <section className="bg-gradient-luxury border-b border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <h1 className="mt-3 font-display text-4xl lg:text-6xl text-primary">VIP & Executive Gifting</h1>
          <div className="gold-divider mt-4" />
          <p className="mt-5 text-muted-foreground max-w-2xl">Distinguished gifts for distinguished professionals and executives.</p>
        </div>
      </section>
