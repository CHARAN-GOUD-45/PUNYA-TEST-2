...
export const Route = createFileRoute("/tech-gifts")({
  head: () => ({
    meta: [
      { title: "Tech Gifts — Punya Ventures" },
      { name: "description", content: "Premium technology and innovative tech gifting solutions." },
    ],
  }),
  component: TechGiftsPage,
});

function TechGiftsPage() {
  const [sort, setSort] = useState("featured");

  const filtered = useMemo(() => {
    let r = products.filter((p) => p.occasion.includes("Tech") || p.name.toLowerCase().includes("tech"));
    ...
    <div>
      <section className="bg-gradient-luxury border-b border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <h1 className="mt-3 font-display text-4xl lg:text-6xl text-primary">Tech Gifts</h1>
          <div className="gold-divider mt-4" />
          <p className="mt-5 text-muted-foreground max-w-2xl">Innovative technology and cutting-edge gadgets for tech enthusiasts.</p>
        </div>
      </section>
