...
export const Route = createFileRoute("/event-seminar-gifts")({
  head: () => ({
    meta: [
      { title: "Event & Seminar Gifts — Punya Ventures" },
      { name: "description", content: "Event and seminar gifting solutions for corporate occasions." },
    ],
  }),
  component: EventSeminarGiftsPage,
});

function EventSeminarGiftsPage() {
  const [sort, setSort] = useState("featured");

  const filtered = useMemo(() => {
    let r = products.filter((p) => p.occasion.includes("Seminar") || p.occasion.includes("Event") || p.badge === "Premium");
    ...
    <div>
      <section className="bg-gradient-luxury border-b border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <h1 className="mt-3 font-display text-4xl lg:text-6xl text-primary">Event & Seminar Gifts</h1>
          <div className="gold-divider mt-4" />
          <p className="mt-5 text-muted-foreground max-w-2xl">Curated gift solutions for events, conferences, and seminars.</p>
        </div>
      </section>
