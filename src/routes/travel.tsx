...
export const Route = createFileRoute("/travel")({
  head: () => ({
    meta: [
      { title: "Travel — Punya Ventures" },
      { name: "description", content: "Premium travel and adventure gifting solutions." },
    ],
  }),
...
function TravelPage() {
  ...
  const filtered = useMemo(() => {
    let r = products.filter((p) => p.name.toLowerCase().includes("travel") || p.name.toLowerCase().includes("bottle"));
    ...
    <div>
      <section className="bg-gradient-luxury border-b border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <h1 className="mt-3 font-display text-4xl lg:text-6xl text-primary">Travel</h1>
          <div className="gold-divider mt-4" />
          <p className="mt-5 text-muted-foreground max-w-2xl">Premium travel accessories for the discerning traveler.</p>
        </div>
      </section>
